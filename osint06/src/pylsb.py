#!/usr/bin/env python3
# Taken from: https://github.com/RenanTKN/pylsb/blob/master/pylsb.py
...
parser.add_option(
    ...
    "--write",
    dest="write",
    help="Write the data on the image",
    action="store_true",
    default=False,
)
parser.add_option(
...
parser.add_option("-t", "--text", dest="text", help="Text to be added on the image")
parser.add_option("-f", "--file", dest="file", help="Text to be added on the image")
parser.add_option(
    "-s",
    "--show",
    dest="show",
...
parser.add_option(
    ...
    "--printable",
    dest="printable",
    help="Outputs only printable chars",
    action="store_true",
    default=False,
)
...
def validateOptions():
    ...
    if options.write:
        if options.text == None and options.file == None:
            raise Exception("In write mode you need to enter a --text or a --file")
        if options.text != None and options.file != None:
